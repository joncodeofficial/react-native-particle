...
- [ ] **2.5D transform matrix** — Add a lightweight transform model for particles so renderers can simulate rotation in `X`, `Y`, and `Z` using a per-particle transform matrix or equivalent projected representation. Even in a 2D engine, this unlocks card-flip confetti, tilting debris, and more convincing perspective motion.
...
- [ ] **Bezier size/alpha curves** — Add optional cubic bezier support for advanced authoring once the simple easing presets have settled. This would allow custom curve shaping beyond `linear`, `easeIn`, `easeOut`, and `pulse`.
